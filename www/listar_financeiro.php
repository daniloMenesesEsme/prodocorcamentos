...
try {
    ...
    // Receita do mês (orçamentos)
    $stmt = $pdo->prepare("SELECT COALESCE(SUM(total), 0) as receita FROM orcamentos WHERE usuario_id = ? AND MONTH(criado_em) = MONTH(NOW()) AND YEAR(criado_em) = YEAR(NOW())");
    ...
    // Lista de despesas (últimas 50)
    $stmt = $pdo->prepare("SELECT * FROM despesas WHERE usuario_id = ? ORDER BY data_despesa DESC, criado_em DESC LIMIT 50");
    $stmt->execute([$usuario_id]);
