...
try {
    ...
    $stmt = $conn->prepare("SELECT id, cliente, itens, total, tipo, criado_em FROM orcamentos WHERE $where_orc ORDER BY criado_em DESC");
    ...
    // Top clientes
    $stmt = $conn->prepare("SELECT cliente, COUNT(*) as qtd, COALESCE(SUM(total), 0) as total FROM orcamentos WHERE $where_orc GROUP BY cliente ORDER BY total DESC LIMIT 5");
    $stmt->execute($params_orc);
